...
const GroupChatListScreen = ({ navigation, route }) => {
  const { user } = useUser();
  const [chats, setChats] = useState(mockGroupChats);
  const [searchQuery, setSearchQuery] = useState('');
  const [sortBy, setSortBy] = useState('priority'); // priority, unread, recent
  const [showSortMenu, setShowSortMenu] = useState(false);
  const [filteredChats, setFilteredChats] = useState(mockGroupChats);
  ...
  const filterChats = (query) => {
    ...
    if (user?.role === 'student') {
      filtered = filtered.filter(chat => chat.isStudent);
    } else if (user?.role === 'teacher') {
      filtered = filtered.filter(chat => chat.isTeacher);
    }
  ...
  const getPriorityColor = (priority) => {
    switch (priority) {
      case 1: return '#4CAF50'; // Green - Low
      case 2: return '#FF9800'; // Orange - Medium
      case 3: return '#F44336'; // Red - High
      default: return '#999';
    }
  };
    ...
    <TouchableOpacity
      ...
      onLongPress={() => Alert.alert(
        ...
        [
          {
            text: 'Priority: Low',
            onPress: () => handlePriorityChange(item.id, 1),
          },
          {
            text: 'Priority: Medium',
            onPress: () => handlePriorityChange(item.id, 2),
          },
          {
            text: 'Priority: High',
            onPress: () => handlePriorityChange(item.id, 3),
          },
      ...
      <View style={styles.sortContainer}>
        ...
        {showSortMenu && (
          <View style={styles.sortMenu}>
            ...
            <TouchableOpacity
              style={[styles.sortOption, sortBy === 'unread' && styles.sortOptionActive]}
